/**
 * Refuerzo opcional con LLM para interpretar la consulta.
 * Nunca es obligatorio: si falla o está off, gana el parseo por reglas.
 *
...
export async function interpretarConsultaConLlm(fraseUsuario) {
    ...
    const system = [
        "Eres un parser de búsquedas de compras públicas en Chile.",
        "Devuelve SOLO un JSON válido con estas claves:",
        'textoSemantico (string: qué producto/servicio buscan, sin montos ni regiones),',
        "montoMaximo (number CLP o null),",
        "montoMinimo (number CLP o null),",
        "regionEtiqueta (string corta tipo Metropolitana/Valparaíso o null),",
        "estado (string o null).",
        "No inventes filtros que el usuario no pidió.",
        "Millones → multiplica por 1000000 (ej. 5 millones = 5000000).",
    ].join(" ");
